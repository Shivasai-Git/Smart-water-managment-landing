export function buildIsoSvgContent(): string {
  ...
  let g = `<defs>
    <linearGradient id="tankWaterGrad" x1="0" y1="0" x2="0" y2="1">
      ...
      <stop offset="25%" stop-color="#18BFF2"/>
  ...
  for (let i = -2.5; i <= 13; i += 1.5)
    grid += ln(P(i, -2.5, 0), P(i, 13, 0), '#0D2D3E', 0.7) + ln(P(-2.5, i, 0), P(13, i, 0), '#0D2D3E', 0.7);
  g += layer(`<g opacity=".55">${grid}</g>`, 0);

  /* --- plinth + floor --- */
  g += layer(
    box(-0.5, -0.5, -0.5, 11, 11, 0.5, '#061D29', '#04151E', '#020C12', ST) +
      pg([P(0, 0, 0), P(10, 0, 0), P(10, 10, 0), P(0, 10, 0)], '#082535', ST) +
      (function () {
        let t = '';
        for (let i = 2; i < 10; i += 2) {
          t += ln(P(i, 0, 0), P(i, 10, 0), '#0D3347', 0.7) + ln(P(0, i, 0), P(10, i, 0), '#0D3347', 0.7);
        }
        return t;
      })(),
    0.08
  ...
  g += layer(
    box(0, -0.28, 0, 10, 0.28, 6, '#0E364C', '#0A2A3C', '#0B2C3F', ST) +
      box(-0.28, 0, 0, 0.28, 10, 6, '#0E364C', '#0B2D40', '#0A2A3C', ST) +
      (function () {
        let t = '';
        for (let i = 1; i < 10; i++) {
          t += ln(P(i, 0, 0), P(i, 0, 6), '#12415C', 0.6) + ln(P(0, i, 0), P(0, i, 6), '#12415C', 0.6);
        }
        for (let k = 1; k < 6; k++) {
          t += ln(P(0, 0, k), P(10, 0, k), '#12415C', 0.6) + ln(P(0, 0, k), P(0, 10, k), '#12415C', 0.6);
        }
        return t;
      })(),
    0.16
  ...
  /* --- indoor pipework (Thick physical pipe with active cyan stream) --- */
  ...
  const pipe = (a: [number, number], b: [number, number]) =>
    ln(a, b, '#061B21', 10) + ln(a, b, '#0C2B36', 7) + ln(a, b, AQ, 3.6, 'pipeflow');
  g += layer(pipe(rTop, rBot) + pipe(rBot, bA1) + pipe(bA1, bA2) + pipe(rBot, bB1) + pipe(bB1, bB2), 0.5);

  /* --- indoor devices --- */
  g += layer(
    cyl(8.8, 2.3, 1.8, 3.7, 0.5, '#0E384F', '#164B68', ST) +
      `<circle cx="${(geyC[0] + 13).toFixed(1)}" cy="${(geyC[1] - 30).toFixed(1)}" r="3.6" fill="${AQ}" class="node"/>` +
      box(5.8, 8.3, 0, 1.3, 1.2, 1.9, '#103F59', '#0C3146', '#092738', ST) +
      `<rect x="${(roC[0] - 12).toFixed(1)}" y="${(roC[1] + 7).toFixed(1)}" width="24" height="7" rx="3" fill="${AQ}" opacity=".75"/>` +
      `<circle cx="${(roC[0] + 15).toFixed(1)}" cy="${(roC[1] - 3).toFixed(1)}" r="3.6" fill="${AQ}" class="node"/>` +
      box(-0.3, 6.7, 1.6, 0.34, 1.3, 1, '#134966', '#0E364C', '#0B2C3F', ST) +
  ...
  g += layer(
    pg(
      ...
      pg([R64(10.5, -0.5), R64(10.5, CUT), R6(10.5, CUT), R6(10.5, -0.5)], '#082130', ST) +
      pg([R64(10.5, CUT), R64(CUT, CUT), R6(CUT, CUT), R6(10.5, CUT)], '#051822', ST) +
      pg([R64(CUT, CUT), R64(CUT, 10.5), R6(CUT, 10.5), R6(CUT, CUT)], '#082130', ST) +
      pg([R64(CUT, 10.5), R64(-0.5, 10.5), R6(-0.5, 10.5), R6(CUT, 10.5)], '#051822', ST) +
      box(-0.5, -0.5, 6.4, 11, 0.3, 0.6, '#103A53', '#0B293C', '#0B293C', ST) +
      box(-0.5, -0.5, 6.4, 0.3, 11, 0.6, '#103A53', '#0B293C', '#0B293C', ST) +
  ...
  [
    ...
    legs += ln(P(TKX + a, TKY + b, 6.4), P(TKX + a, TKY + b, 7.6), '#1B4764', 3.6);
  });
  ...
  g += layer(
    legs +
      ln(P(7.4, 1.8, 7.15), P(TKX, 1.8, 7.15), '#061B21', 9) +
      ln(P(TKX, 1.8, 7.15), [Cb[0], Cb[1] + 3], '#061B21', 9) +
      ln(P(7.4, 1.8, 7.15), P(TKX, 1.8, 7.15), AQ, 3.2, 'pipeflow') +
      `<clipPath id="tankClip"><path d="${tankBody}"/></clipPath>` +
      cyl(TKX, TKY, 7.5, 11.3, TR, 'rgba(24,191,242,.28)', 'rgba(114,228,255,.35)', '#18BFF2') +
      `<g clip-path="url(#tankClip)">
       <rect id="wRect" x="${(Ct[0] - TRX).toFixed(1)}" y="${botY.toFixed(1)}" width="${(TRX * 2).toFixed(1)}" height="4" fill="url(#tankWaterGrad)" opacity=".88"/>
       <rect id="wRect2" x="${(Ct[0] - TRX).toFixed(1)}" y="${botY.toFixed(1)}" width="${(TRX * 2).toFixed(1)}" height="4" fill="#72E4FF" opacity=".35"/>
     </g>` +
      `<ellipse id="wSurf" cx="${Ct[0].toFixed(1)}" cy="${botY.toFixed(1)}" rx="${(TRX - 5).toFixed(1)}" ry="${(TRY - 4).toFixed(1)}" fill="#72E4FF" stroke="#CAF0F8" stroke-width="1.3" opacity="0"/>` +
      `<ellipse id="wRipple" cx="${Ct[0].toFixed(1)}" cy="${botY.toFixed(1)}" rx="${(TRX - 9).toFixed(1)}" ry="${(TRY - 6).toFixed(1)}" fill="none" stroke="#CAF0F8" stroke-width="2" opacity="0"/>` +
      `<path d="${tankBody}" fill="none" stroke="#72E4FF" stroke-width="2.2" opacity=".78"/>` +
      (function () {
        let r = '';
        for (let i = 1; i < 4; i++) {
      ...
      `<path d="M${Ct[0].toFixed(1)},${(Ct[1] - 2).toFixed(1)} L${(Ct[0] - 13).toFixed(1)},${(Ct[1] + 30).toFixed(1)} L${(Ct[0] + 13).toFixed(1)},${(Ct[1] + 30).toFixed(1)} Z" fill="${AQ}" opacity=".15"/>` +
      `<rect x="${(Ct[0] - 15).toFixed(1)}" y="${(Ct[1] - 16).toFixed(1)}" width="30" height="13" rx="5" fill="${AQ}"/>` +
      `<circle cx="${Ct[0].toFixed(1)}" cy="${(Ct[1] - 9.5).toFixed(1)}" r="3" fill="#031014" class="node"/>` +
  ...
  /* --- pump --- */
  g += layer(
    box(6.5, 1.1, 6.4, 1.9, 1.4, 0.5, '#123D57', '#0C2D42', '#092435', ST) +
      cyl(7.2, 1.8, 6.9, 7.95, 0.58, '#154866', '#1E5E84', ST) +
      box(8.3, 1.25, 6.9, 0.7, 1.15, 0.85, '#123D57', '#0D3147', '#0A273A', ST) +
  ...
  const PX = 800,
    ...
    PH = 224;
  g += layer(
    `<rect x="${PX}" y="${PY}" width="${PW}" height="${PH}" rx="17" fill="#041620" stroke="${ST}" stroke-width="1.6"/>
     <rect x="${PX + 7}" y="${PY + 7}" width="${PW - 14}" height="${PH - 14}" rx="12" fill="#072230"/>
     <rect x="${PX + 42}" y="${PY + 12}" width="28" height="4" rx="2" fill="#041620"/>
     <text x="${PX + 15}" y="${PY + 38}" fill="${STL}" font-family="IBM Plex Mono, monospace" font-size="8" letter-spacing="1">TANK 01</text>
     <path d="M${PX + 22},${PY + 94} A38,38 0 0 1 ${PX + 90},${PY + 94}" fill="none" stroke="#0D3348" stroke-width="7" stroke-linecap="round"/>
  ...
  /* --- digital wireless telemetry links (Thin, dashed, muted teal/saffron) --- */
  ...
  const arc = (a: [number, number], b: [number, number], c: string) => {
    ...
    return `<path d="M${a[0].toFixed(1)},${a[1].toFixed(1)} Q${mx.toFixed(1)},${my.toFixed(1)} ${b[0].toFixed(1)},${b[1].toFixed(1)}" fill="none" stroke="${c}" stroke-width="1.5" class="sig" opacity=".8"/>`;
  };
  g += layer(
